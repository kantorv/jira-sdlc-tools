...
### The cautionary tale, from this very change

Run against JST-271 after all four sub-tasks had been reviewed, approved and
merged, that grep found exactly one survivor —
...
the prefix list *and* the `+`→`*` quantifier. (Fixed in the same commit that
added this guide.)
