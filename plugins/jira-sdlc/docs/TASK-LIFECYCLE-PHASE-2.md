...
sequenceDiagram
    ...
    par every leaf — own worktree (truly parallel)
        ...
        activate Executor
        Executor->>JIRA: check_assignee.sh --role executor<br/>(is this assigned to me?)
        JIRA-->>Executor: yes, continue
...
- **Identity first, then ownership** — before anything else, the executor
  **authenticates as itself** (per-request `--role executor`), so every Jira write
  ...
  *before* it has touched anything, printing the ready-to-paste
  ...
  [`../skills/_shared/jira-api-reference.md` §10](../skills/_shared/jira-api-reference.md).)
