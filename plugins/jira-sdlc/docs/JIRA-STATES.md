...
| **[GitHub Actions](STATE-TRANSITIONS-WITH-GITHUB-ACTIONS.md)** [^ci] | ❌ none ships | ✅ `jira_issue_transition_on_branch.yml` — on `create` of any issue branch (`feature/*`, `bugfix/*`, `chore/*`, `hotfix/*`), and only from `<STATUS_TODO>` | ✅ `jira_issue_transition_on_pr_open.yml` — on PR opened/reopened, skipped if already In Review or Done | ✅ `jira_issue_transition_on_merge.yml` — on PR closed-as-merged, skipped if already Done |
