...
Reads <PROJECT-KEY> from jira-tools-plugin.env (override with --project
or $PROJECT_KEY); the project isn't passed to acli view but is printed
for confirmation.

Usage:
    acli-list-subtasks.py --parent <PARENT-KEY> [--env ./jira-tools-plugin.env] [--json]
"""
...
def resolve_project(env_path: str) -> str:
    ...
    for p in (Path(env_path), Path("jira-tools-plugin.env"),
              Path("../jira-tools-plugin.env")):
        if p.is_file():
...
def main() -> int:
    ...
    ap.add_argument("--env", default="./jira-tools-plugin.env",
                    help="Path to jira-tools-plugin.env (for the project label only)")
