...
#
# POSIX twin of ../win/collect_feature.ps1 — same CLI, same stdout/stderr split,
# same exit codes. The heavy JSON roll-up runs in python3 (advanced parsing +
# nested aggregation, mirroring sync_conversations.sh's python core and reusing
# jq via collect_run); this bash shim only plumbs the sibling paths + config in.
#
...
export CF_SCRIPT_DIR="$SCRIPT_DIR"

PROG=$(cat <<'PYEOF'
import sys, os, re, json, subprocess, shutil

ACLI_TIMEOUT_SEC = 300
SKILL_DIR = os.environ.get('CF_SCRIPT_DIR', '')
...
# ---- sibling scripts (same posix/ folder) -----------------------------------
...
# read the same way (env files, not the process env). A path under the worktrees
...
def feature_records(fkey, soft):
    ...
    if sentinel_idx >= 0:
        for ln in sync_out[:sentinel_idx]:
            note(ln)
    else:
        for ln in sync_out:
            note(ln)
...
def new_aggregate(records):
    ag_in = ag_out = ag_cr = ag_cw = 0
    ag_turns = ag_side = ag_calls = ag_errs = 0
    mdl_set = []
    skl_set = []
    key_set = []
    by_skill = {}   # skill -> token acc
    by_skill_order = []
    ...
    for r in records:
        ...
        if r['key_status'] != 'expected' and r['key_status'] != 'given':
            continue
        analyzed += 1
        ag_in += int(r['tokens']['in'])
        ag_out += int(r['tokens']['out'])
        ag_cr += int(r['tokens']['cache_read'])
        ag_cw += int(r['tokens']['cache_write'])
        ag_turns += int(r['skill_turns'] or 0)
        ag_side += int(r['sidechain_turns'] or 0)
        ag_calls += int(r['tool_calls'] or 0)
        ag_errs += int(r['tool_errors'] or 0)

        sk = r['skill'] if r['skill'] else '(no skill)'
        if sk not in by_skill:
            by_skill[sk] = acc(sk)
            by_skill_order.append(sk)
        add_to(by_skill[sk], r)

        pv = r['provenance'] if r['provenance'] else 'unknown'
        if pv not in by_prov:
            by_prov[pv] = acc(pv)
        ...
        l = parse_iso(r['last_ts'])
        if l is not None and (max_last is None or l > max_last):
            max_last = l
            max_last_str = r['last_ts']

    ag_tot = ag_in + ag_out + ag_cr + ag_cw
    span_s = None
    if min_first is not None and max_last is not None:
        span_s = round((max_last - min_first).total_seconds(), 1)

    by_skill_arr = []
    for k in by_skill_order:
        a = by_skill[k]
        by_skill_arr.append({'skill': k, 'conversations': a['conversations'],
                             'tokens': {'in': a['in'], 'out': a['out'], 'cache_read': a['cache_read'],
                                        'cache_write': a['cache_write'], 'total': a['total']}})
    by_prov_arr = []
    for k in by_prov_order:
        b = by_prov[k]
    ...
    return {
        ...
        'tokens': {'in': ag_in, 'out': ag_out, 'cache_read': ag_cr, 'cache_write': ag_cw, 'total': ag_tot},
        'skill_turns': ag_turns,
        'sidechain_turns': ag_side,
        'tool_calls': ag_calls,
        'tool_errors': ag_errs,
        'timeframe': {'first_ts': min_first_str, 'last_ts': max_last_str, 'span_s': span_s},
...
# =============================================================================
# SINGLE-STEP  (no sub-tasks) — flat feature-report@2
# =============================================================================
...
for sub in detect['subtasks']:
    child_raw = feature_records(sub['key'], True)
    child_kept = []
    for r in child_raw:
