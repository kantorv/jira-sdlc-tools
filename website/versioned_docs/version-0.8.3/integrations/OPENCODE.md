...
- **`allowed-tools` is also ignored.** The skills declare `allowed-tools: Bash, Read, Grep, Glob[, Edit], Write, AskUserQuestion` in frontmatter (`Edit` on the executor only); OpenCode does not read this field — which also means the Claude-side `AskUserQuestion` name in the list is inert here by construction, not a compatibility risk. Tool access during a run is governed instead by OpenCode's own `permission` config (`bash`, `edit`, `read`, `webfetch`, etc. → `allow` / `ask` / `deny`). Expect approval prompts for `bash` / `edit` / `write` unless your config allows them up front.
...
- **Verified in OpenCode.** The install and invocation steps were run end-to-end in OpenCode. The config keys, the frontmatter-recognition list, the `subtask` semantics, and the `permission`-vs-`disable-model-invocation` distinction were confirmed working. `opencode.json` must be in the project root — OpenCode will not read it from any subdirectory.
