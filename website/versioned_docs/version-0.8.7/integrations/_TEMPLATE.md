...
> `NVIDIA-NIM.md`), give it an entry in `scripts/docs-url-map.json` and run
> `python3 scripts/repair-doc-links.py` to write its front matter — this
> template carries none, because a `_`-prefixed file is not published — then
> add a row to the summary table in the
> root [`INTEGRATIONS.md`](https://github.com/kantorv/jira-sdlc-tools/blob/main/INTEGRATIONS.md). Delete this
